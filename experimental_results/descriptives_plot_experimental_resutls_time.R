...
ser<- as.data.frame(read_excel("experimental_results/simulated_experiments_results_time_low.xlsx"))

ser$iter <- 2*ser$iter-2
...
as.data.frame(ser %>% group_by(method, prior) %>%summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))))



counts_rep = as.data.frame(ser %>% group_by(method, prior) %>%summarise(mean(iter)))
counts_rep
...
sorted_df_report <- ser_df_method_prior_grouped[
  ...
  c("method","prior","iter_Mean","iter_SD","PE_dof_Mean","PE_dof_SD","PE_V_Mean","PE_V_SD", 
    "PE_ALL_Mean", "PE_ALL_SD","ESS_Mean","ESS_SD","MCSE_Mean","MCSE_SD","Geweke_Mean","Geweke_SD")]

sorted_df_report

sorted_df_report = data.frame(lapply(sorted_df_report, function(y) if(is.numeric(y)) round(y, 3) else y)) 
write_xlsx(sorted_df_report,"experimental_results/sorted_df_report_time_high_dim.xlsx")
...
ser_low <- as.data.frame(read_excel("experimental_results/simulated_experiments_results_time_low.xlsx"))
ser_high <- as.data.frame(read_excel("experimental_results/simulated_experiments_results_time_high.xlsx"))

ser_all = rbind(ser_low,ser_high)

ser_all$dof = paste0('dof = ',ser_all$dof)


plot_ser_df <- ser_all[,c(1:5,7)]
...
levels(plot_ser_df$method_prior) <- c('ML', 'RWM_Gibbs_gamma',
                                      'Slice_Gibbs_inverse_gamma',
