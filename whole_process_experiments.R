    ...
    for (dof in dof_list[dof_list>d]){
      for (iter in 1:10){
        ...
        df_all_iter <- data.frame(obs = obs, d = d, dof = dof, 
                                  ...
                                  time = 0,PE_dof = PE_df_Newton_Raphson,
        ...
        for (alg in mcmc_algos){
          for (pr in priors_dist){
            start.time <- Sys.time()
            MwG <- metropolis_within_gibbs(model_data, metropolis_alg = alg, prior = pr)
            end.time <- Sys.time()
            mwg_time.taken <- as.numeric(difftime(end.time,  start.time, units='mins'))
            ...
            df_all_iter <- rbind(df_all_iter, list(obs,d, dof, paste0('MwG_',alg),
                                                   pr, mwg_time.taken, PE_df_mwg,
                                                   PE_V_mwg, ess_df_mwg, geweke_df_mwg))
          }
          }

        for (pr in priors_dist){
          if (d<30){
            result_stan <- try(withTimeout({
            start.time <- Sys.time()
            ...
            return(rbind(df_all_iter, list(obs,d, dof, 'stan_all_model',
                                                   pr, stan_all.taken, PE_df_stan,
                                                   PE_V_stan, ess_df_stan, geweke_df_stan)))
          }, timeout = 3600, onTimeout = "silent"), silent = TRUE)
          
          # Check if the computation was finished or timed out
          if (is(result_stan, "try-error")) {
            print('stan_all_model')
            stan_all.taken <- 60
            PE_df_stan<-NA
            PE_V_stan<-NA
